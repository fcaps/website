...
const d = new Date()
const timeFilter = 6 // 6 Months is the default value
const minusTimeFilter = d.setMonth(d.getMonth() - timeFilter)
const currentDate = new Date(minusTimeFilter).toISOString()
...
function clanUpdate () {
    ...
    const clanDelete = document.querySelectorAll('.clanDelete')
    ...
    const next100Players = clanList.length - (pageNumber * 100)
...
function pressEnter (event) {
    ...
    if (inputText === '') {
        ...
        const searchName = clanList.filter(element => element[1].tag.toLowerCase().match(regex))
...
function findPlayer (userInput) {
    clanOneJSON(currentClan)
        .then(() => {
            ...
            const searchClan = clanList.findIndex(element => element[1].tag.toLowerCase() === userInput.toLowerCase())
            if (searchClan !== -1) {
...
document.querySelector('#clearSearch').addEventListener('click', () => {
    ...
    const clanDelete = document.querySelectorAll('.clanDeleteSearch')
